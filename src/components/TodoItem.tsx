  ...
  return (
    <div className={`md:flex space-x-5 m-5 border border-blue-300 justify-between ${todo.isComplete ? 'bg-green-300' : 'bg-blue-200'} `}>
        <div className='w-5/6 flex md:text-2xl'>
          ...
          {editMode 
           ?
           <textarea  className='grow w-full p-2 m-5 self-start rounded-3xl resize-none' onChange={inputHandler} value={value}></textarea>      
           :
        ...
        <button onClick={() => setEditMode(true)} className='border border-gray-800 p-1 bg-gray-300'>edit</button>
        </>}
        </div>
      </div>
  )
